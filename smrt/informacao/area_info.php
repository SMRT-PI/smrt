...
$sql = "SELECT area_info.id as materia,area_info.titulo,area_info.descricao,area_info.conteudo,area_info.capa,"
        . "area_info.autor,area_info.dataa,usuario.id,usuario.nome,usuario.sobrenome,"
        . "date_format(dataa, '%d-%m-%Y %H:%i:%s') as dataa FROM"
        . " area_info inner join usuario on area_info.autor = usuario.id order by materia Desc;";
    ...
    <div class="list-group d-flex w-100">
        ...
        if (mysqli_num_rows($resultado) > 0) {
            while ($linha = mysqli_fetch_array($resultado)) {
                ?>
                <a class="hover-zoom list-group-item my-1 list-group-item-action flex-column align-items-start img-fluid" href="conteudo.php?id=<?= $linha['materia'] ?>" style="background-image: url(http://localhost/smrt/img/<?php echo $linha['capa'] ?>);" >
