$(function(){
   ...
   $('input[name=next1]').click(function(){
       var array = formulario.serializeArray();
            if (array[0].value == '' || array[1].value == ''|| array[2].value == ''){
                $('.resp').html('<div class="erros"><p>Preencha todos os dados para prosseguir!</p></div>');
            }else{
                $('.resp').html('');
                next($(this));
            }
   ...
   $('input[type=submit]').click(function(evento){
            ...
            if (array[3].value == '' || array[4].value == ''){
                $('.resp').html('<div class="erros"><p>Preencha todos os dados para se cadastrar!</p></div>');
            }else{
//                $('.resp').html('<div class="ok"><p>A partir daqui validaremos com php!</p></div>');
                $.ajax({
                   method: 'post',
                   url: 'cadastrar.php',
